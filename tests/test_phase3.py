...
def test_template_share(client: APIClient) -> bool:
    ...
    # 1. 创建测试模板
    print("1. 创建测试模板...")
    template = client.post("/templates", TEST_TEMPLATE)
    if not template:
        print("\u2717 创建模板失败")
        return False
    ...
    # 4. 获取分享给我的模板
    print("4. 获取分享给我的模板...")
    shared_templates = client.get("/templates/shared/me")
    ...
    # 5. 获取模板分享详情
    print("5. 获取模板分享详情...")
    share_details = client.get(f"/templates/{template_id}/shares")
    ...
    # 6. 清理测试数据
    print("6. 清理测试数据...")
    if client.delete(f"/templates/{template_id}"):
        print("\u2713 删除模板成功")
    else:
        print("\u26a0 删除模板失败")
...
def test_version_diff(client: APIClient) -> bool:
    ...
    # 3. 获取版本列表
    print("3. 获取版本列表...")
    versions = client.get(f"/templates/{template_id}/versions")
    ...
    diff = client.get(
        f"/templates/{template_id}/versions/diff",
...
def test_async_export(client: APIClient) -> bool:
    ...
    print(f"\u2713 创建导出任务成功: ID={task_id}, 状态={export_task.get('status')}")

    # 2. 获取导出任务列表
    print("2. 获取导出任务列表...")
    tasks = client.get("/async-export/tasks")
    if not tasks:
        print("\u2717 获取导出任务列表失败")
        return False
    ...
    # 3. 获取导出任务详情
    print("3. 获取导出任务详情...")
    task_detail = client.get(f"/async-export/task/{task_id}")
    if not task_detail:
        print("\u2717 获取导出任务详情失败")
        return False
