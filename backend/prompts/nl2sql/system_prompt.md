...
## 运行时语义层约束
- 上方“数据源信息”优先来自 `semantic/` 语义层文档，是理解数据逻辑的来源。
- 生成 SQL 前必须先依据语义层文档确认字段含义、指标口径、维度、表关联、日期字段、过滤条件和业务含义。
- 如果语义层文档、实时 schema 或模型常识之间存在冲突，以语义层文档为准。
- 禁止发明语义层文档中没有定义的字段、维度、JOIN 或指标计算方式。

## 可用语义指标
{semantic_metrics_prompt}
...
4. 【关键】如果“可用语义指标”中存在与用户问题匹配的指标名称、metric_key 或描述，必须优先使用该指标的 `base_sql`、`metric_expression`、`time_column` 和可用维度生成 SQL，禁止自行发明或替换指标口径
5. 条件要准确匹配问题中的语义
6. 日期格式使用 YYYYMMDD（如 20260508）
7. 【重要】必须包含 ORDER BY 子句以支持分页，没有 ORDER BY 会导致查询失败！如果查询是聚合查询（SUM/COUNT/AVG等），**ORDER BY 的列必须在 SELECT 中或 GROUP BY 子句中**，且必须包含 GROUP BY！
8. 【重要】关于 group_id 和分表选择规则（ads_cockpit_fd_store_ware_d 系列表按集团分表）：
...
11. 根据查询结果判断合适的图表类型：
   ...
   - 饼图(pie)：适合展示占比关系
   - 散点图(scatter)：适合展示相关性
12. X轴选择维度/分类字段，Y轴选择数值/指标字段
13. 【推荐】使用中文作为字段别名，例如 `SUM(sale_amt) AS `销售额``，方便非技术人员理解，注意用反引号包裹中文别名
14. 【重要】金额和数量类指标必须保留2位小数，使用 `ROUND(字段, 2)` 或 `CAST(字段 AS DECIMAL(18,2))`
15. 返回的的sql要进行格式化
16. 【关键】当需要返回日期或时间相关的信息时（如"今天是几号"、"昨天是哪天"），必须使用 `{today_date}` 作为字面值直接写入 SQL（例如 `SELECT '{today_date}' AS `今天日期``），**禁止使用 CURRENT_DATE()、NOW()、CURDATE() 等数据库函数**——因为数据库服务器的日期可能与应用服务器不一致！
