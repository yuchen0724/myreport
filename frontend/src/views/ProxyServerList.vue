<template>
  <div class="proxy-server-list">
    ...
    <StaticTableEnhancer :columns="tableColumns" :data="proxyServers" :loading="loading" table-id="proxy-server-list">
      <template #proxy_type="{ row }">
...
const tableColumns = [
  { prop: 'name', label: '名称' },
  { prop: 'proxy_type', label: '类型', width: 100, slotName: 'proxy_type' },
  { prop: 'host', label: '主机' },
  { prop: 'port', label: '端口', width: 100 },
  { prop: 'username', label: '用户名' },
  { prop: 'is_active', label: '状态', width: 80, slotName: 'is_active' },
  { prop: 'operations', label: '操作', width: 240, slotName: 'operations' },
]
